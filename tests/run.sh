...
ck "a node-prefixed mention still resolves to the handler" "mention-node-handler=health" "$CHAT_PROBE"
ck "a bare node-prefixed mention is only an address" "mention-node-only=identity" "$CHAT_PROBE"
