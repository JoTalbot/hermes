...
| 3 | **Android control plane** | **PASS** | 9119 open from outside (check-host, 6/6 vantage points) and served over HTTPS-less plain port by the owner's explicit decision; all state lives on the server, the phone is only a client |
