...
INTENT_RULES: list[tuple[str, str, str, str]] = [
    ...
    (r"сервис|юнит|systemd|упал|failed|не работа|остановлен", "host-health", "сервисы",
     "services"),
    # FACT (2026-09-17): история писалась, но «падало три раза по одной причине» система не
    # отвечала — разбор каждый раз начинался с нуля.
    (r"повторяющ|повторяется|одн[ау] и та же причина|падает снова|падает каждый раз|опять упал",
     "host-health", "повторяющиеся сбои", "repeats"),
